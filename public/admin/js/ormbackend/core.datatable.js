		...
		var Plugin = {
			...
			extractTable: function() {
				...
				var headers = $(datatable).find('tr:first-child th').get().map(function(cell, i) {
					...
					if (typeof $(cell).data('type') !== 'undefined') column.type = $(cell).data('type');
					if (typeof $(cell).data('format') !== 'undefined') column.format = $(cell).data('format');
					if (typeof $(cell).data('overflow') !== 'undefined') column.overflow = $(cell).data('overflow');
					if (typeof $(cell).data('textalign') !== 'undefined') column.textAlign = $(cell).data('textalign');
					if (typeof $(cell).data('width') !== 'undefined') column.width = $(cell).data('width');
					if (typeof $(cell).data('sortable') !== 'undefined') column.sortable = $(cell).data('sortable');
					if (typeof $(cell).data('autohide') !== 'undefined') column.autoHide = $(cell).data('autohide');
					if (typeof $(cell).data('selector') !== 'undefined') column.selector = {class: $(cell).data('autohide')};

					for (var ii in options.columns) {
						if (options.columns[ii].field === field) {
							column = $.extend(true, column, options.columns[ii]);
						}
					}
					columns.push(column);
					return field;
			...
			dataRender: function(action) {
				...
				var afterGetData = function(result) {
					...
					if (options.pagination) {
						if (options.data.serverPaging) {
							// server pagination
							var serverMeta = null;
							if (options.data.type === 'local') {
								// TODO global
								serverMeta = window.pagination;
								Plugin.pagingObject = Plugin.paging(serverMeta, function(ctx, meta) {
									var params = $.deparam(window.location.search ? window.location.search.substr(1) : '');
									params['page'] = serverMeta.perpage != meta.perpage ? 1 : meta.page;
							...
							if (!serverMeta) {
								serverMeta = Plugin.getObject('meta', result || null);
								if (serverMeta !== null) {
				...
				var sortObj = {
					...
					sortClick: function(e) {
						...
						if (options.sortable) {
							...
							sortObj.setIcon();
							$(datatable).trigger(pfx + 'datatable--before-sort', meta);
